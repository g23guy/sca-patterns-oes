...
# Title:       Novell DFS fails due to missing Required-Start
...
sub usingDFS {
	#SDP::Core::printDebug('> usingDFS', 'BEGIN');
	my $RCODE = 0;
	my $PACKAGE_NAME = 'novell-nss';
	my $SERVICE_NAME = 'novell-dfs';
	...
	if ( SDP::SUSE::packageInstalled($PACKAGE_NAME) ) {
		...
		if ( $SERVICE_INFO{'runlevelstatus'} > 0 ) {
			...
			SDP::Core::updateStatus(STATUS_ERROR, "ERROR: usingDFS(): $SERVICE_INFO{'name'} turned off, skipping Required-Start test");
		}
	} else {
		SDP::Core::updateStatus(STATUS_ERROR, "ERROR: usingDFS(): Novell $SERVICE not installed, skipping Required-Start test");
	}
	#SDP::Core::printDebug("< usingDFS", "Returns: $RCODE");
	return $RCODE;
}
