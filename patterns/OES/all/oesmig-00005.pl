...
# Title:       OES2 Linux target needs NSS loaded/healthy
# Description: If the target volume is NSS, the NSS file system should be loaded / healthy and a mount point available.
...
my $FILE_NAME = 'novell-nss.txt';
my $SERVICE_NAME = 'novell-nss';
my %HOST_INFO = SDP::SUSE::getHostInfo();
if ( SDP::SUSE::packageInstalled('novell-nss') && $HOST_INFO{'oesmajor'} == 2 ) {
	...
	} elsif ( $SERVICE_STATUS == 3 ) {
		SDP::Core::updateStatus(STATUS_ERROR, "Unknown Service Run State: $SERVICE_NAME");
	} else {
