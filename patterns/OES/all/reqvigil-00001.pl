...
# Title:       Novell Vigil fails due to missing Required-Start
...
sub usingVigil {
	SDP::Core::printDebug('> usingVigil', 'BEGIN');
	my $RCODE = 0;
	my $PACKAGE_NAME = 'novell-vigil';
	my $SERVICE_NAME = 'novell-vigil';
	...
	if ( SDP::SUSE::packageInstalled($PACKAGE_NAME) ) {
		...
		if ( $SERVICE_INFO{'runlevelstatus'} > 0 ) {
			...
			SDP::Core::updateStatus(STATUS_ERROR, "ERROR: usingVigil(): $SERVICE_INFO{'name'} turned off, skipping Required-Start test");
		}
	} else {
		SDP::Core::updateStatus(STATUS_ERROR, "ERROR: usingVigil(): Novell $SERVICE not installed, skipping Required-Start test");
	}
	SDP::Core::printDebug("< usingVigil", "Returns: $RCODE");
	return $RCODE;
}
